...
def aminoacids(args):
    ...
    # End processing for --by_length
        ...
        # argparse already handles this via str2bool for other subcommands,
        # but aminoacids() may also be called directly (e.g. in tests) with
        # plain strings, so we re-check defensively here. These are kept as
        # local variables (rather than overwriting args.export/args.threeD)
        # so the --by_length branch above, which still expects the original

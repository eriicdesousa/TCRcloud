...
def process_csv_files(folder_path):
    ...
    for file in os.listdir(folder_path):
        if file.endswith(".csv"):
            ...
            df[length_columns] = (
                df[length_columns] * 100 / df[length_columns].sum().sum()
            ).round(1)

            # Compute total percentage for each length and save as TSV
            length_distribution = df[length_columns].sum()
            length_tsv_path = os.path.join(
                analysis_folder, f"{output_name}_length_distribution.tsv"
            )
            ...
            if ace_tools_available:
                tools.display_dataframe_to_user(
                    ...
                    dataframe=top_v_calls[
                        ["v_call", "Total_Percentage", "Mean_Length"]
                    ],
                )
...
def _plot_length_distribution(length_distribution, output_name, analysis_folder):
    ...
    graph_path = os.path.join(
        analysis_folder, f"{output_name}_cdr3_length_distribution.png"
    )
...
def _save_top_v_calls(top_v_calls, output_name, analysis_folder):
    ...
    top_v_calls[["v_call", "Total_Percentage", "Mean_Length"]].to_csv(
